...
export default function StudyAssistantPage() {
  ...
  async function sendMessage(event?: FormEvent) {
    ...
    try {
      const response = await fetch("/ai/study-assistant", {
        method: "POST",
      ...
      setMessages((current) => current.filter((item, index) => !(item.role === "user" && index === current.length - 1)));
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-5xl flex-col px-3 py-4 sm:px-6 sm:py-8">
        <header className="mb-5 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-600 dark:bg-[#3b3b3b] sm:p-6">
          <div className="flex items-start gap-3">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-300">
              <Sparkles size={22} />
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-6">
            {!hasMessages ? (
              <div className="flex min-h-[390px] flex-col items-center justify-center text-center">
                <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-300">
                  <Bot size={32} />
                </div>
                ...
                <div className="mt-6 grid w-full max-w-2xl gap-3 sm:grid-cols-3">
                  {quickPrompts.map(({ label, icon: Icon, text }) => (
                    <button
                      key={label}
                      type="button"
                      onClick={() => useQuickPrompt(text)}
                      className="flex min-h-12 items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 p-3 text-left text-sm font-semibold text-gray-700 transition hover:border-indigo-300 hover:bg-indigo-50 dark:border-gray-600 dark:bg-[#333333] dark:text-gray-100 dark:hover:border-indigo-400 dark:hover:bg-indigo-500/10"
                    >
                      <Icon className="shrink-0 text-indigo-500" size={19} />
                      <span>{label}</span>
                    </button>
                  ))}
                </div>
              ...
              <div className="mx-auto max-w-3xl space-y-5">
                {messages.map((message, index) => (
                  <div key={`${message.role}-${index}`} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                    {message.role === "model" && (
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-300">
                        <Bot size={18} />
                      </div>
                    )}
                    <div className={`max-w-[88%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm leading-6 sm:max-w-[78%] ${message.role === "user" ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-800 dark:bg-[#333333] dark:text-gray-100"}`}>
                      {message.text}
                    </div>
                    {message.role === "user" && (
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-200 text-gray-600 dark:bg-gray-600 dark:text-gray-100">
                        <User size={18} />
                      </div>
                    )}
                  </div>
                ...
                {loading && (
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-300"><Bot size={18} /></div>
                    <div className="rounded-2xl bg-gray-100 px-4 py-3 text-sm text-gray-500 dark:bg-[#333333] dark:text-gray-300">Đang suy nghĩ…</div>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="border-t border-gray-200 p-3 dark:border-gray-600 sm:p-4">
            ...
            <form onSubmit={sendMessage} className="mx-auto flex max-w-3xl items-end gap-2">
              <textarea
                ...
                rows={1}
                className="max-h-36 min-h-12 flex-1 resize-y rounded-xl border border-gray-300 bg-white px-4 py-3 text-base text-gray-900 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-gray-600 dark:bg-[#333333] dark:text-white dark:placeholder:text-gray-400"
                disabled={loading}
              />
              <button
                type="submit"
                ...
                className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <Send size={19} />
              </button>
            </form>
            <p className="mx-auto mt-2 max-w-3xl text-center text-xs text-gray-400">Enter để gửi · Shift + Enter để xuống dòng</p>
          </div>
        </section>
      </div>
    </main>
